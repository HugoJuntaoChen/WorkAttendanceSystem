...
export default {
  ...
  created () {
    this.hello = sessionStorage.getItem('username')
    this.username = sessionStorage.getItem("username");
    this.$axios
      .get(
        `http://127.0.0.1:7001/staff/getsomestaffinfo?username=${sessionStorage.getItem(
          "username"
        )}`
      )
      .then(res => {
  ...
  methods: {
    uploadFile: function (event) {
      ...
      let that = this;
      this.$axios
        .post("http://127.0.0.1:7001/photo/uploadimg", param)
        .then(function (res) {
          obj.photo = res.data;
          obj.username = that.username;
          that.$axios
            .post("http://127.0.0.1:7001/photo/save", obj)
            .then(res => {
              that.$axios
                .get(`http://127.0.0.1:7001/photo/get?${that.username}`)
                .then(res => {
                  console.log(res);
    ...
    postChangeStaffInfo () {
      ...
      this.$axios
        .post("http://127.0.0.1:7001/staff/changestaffinfo", obj)
        .then(res => {
